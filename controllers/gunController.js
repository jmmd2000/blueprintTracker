...
exports.gun_detail = function (req, res, next) {
    ...
    async.parallel({
        ...
        if (results.gun == null) { // No results.
...
exports.gun_create_post = [
    ...
    body('gun_name', 'Name must not be empty.').trim().isLength({
        min: 1
    }).escape(),
...
exports.gun_update_get = function (req, res, next) {
    ...
    async.parallel({
        ...
        function (err, results) {
            ...
            for (var all_a_iter = 0; all_a_iter < results.gun.compatible.length; all_a_iter++) {
                for (var gun_a_iter = 0; gun_a_iter < results.gun.compatible.length; gun_a_iter++) {
                    if (results.gun.compatible[all_a_iter]._id.toString() === results.gun.compatible[gun_a_iter]._id.toString()) {
...
exports.gun_update_post = [
    ...
    body('gun_name', 'Gun name must not be empty.').trim().isLength({
        min: 1
    }).escape(),
    body('gun_des', 'Author must not be empty.').trim().isLength({
        min: 1
    }).escape(),
    body('gun_class', 'Summary must not be empty.').trim().isLength({
        min: 1
    }).escape(),
    body('gun_img', 'ISBN must not be empty').trim().isLength({
        min: 1
    }).escape(),
    ...
    (req, res, next) => {
        ...
        var gun = new Gun({
            ...
            weaponClass: req.body.gun_class,
            image: req.body.gun_img,
            compatible: (typeof req.body.compatible === 'undefined') ? [] : req.body.compatible,
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                for (let i = 0; i < results.gun.compatible.length; i++) {
                    if (results.gun.compatible.indexOf(results.gun.compatible[i]._id) > -1) {
